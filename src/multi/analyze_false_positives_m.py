"""
analyze_false_positives.py — Multi-Class Detector False Positive Analysis

Usage
-----
    python src/multi/analyze_false_positives.py
    python src/multi/analyze_false_positives.py --split test
    python src/multi/analyze_false_positives.py --class_id 0        # hammer only
    python src/multi/analyze_false_positives.py --class_id all
"""
...
DEFAULTS = {
    ...
    "split":      "val",
    "class_id":   "0",
    "out":        str(ROOT / "false_positive_report_multi.json"),
    "output_dir": str(ROOT / "outputs/multi/false_positives"),
}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="False positive analysis for multi-class detector")
...
def main():
    ...
    print(f"\n{'='*56}")
    print(f"  FALSE POSITIVE ANALYSIS  [MULTI]")
    print(f"{'='*56}")
    ...
    for img_path in image_paths:
        ...
        for pi, pred in enumerate(pred_boxes):
            ...
            for gi, gt in enumerate(gt_boxes):
                if gi in matched_gt or pred[0] != gt[0]:
                    continue
        ...
        annotated = img.copy()
        for gt in gt_boxes:
            cv2.rectangle(annotated, (gt[1], gt[2]), (gt[3], gt[4]), GT_COLOUR, 2)
            name = CLASS_NAMES[gt[0]] if gt[0] < len(CLASS_NAMES) else str(gt[0])
            cv2.putText(annotated, f"GT:{name}", (gt[1], gt[2] - 6),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, GT_COLOUR, 2)
        for i, pred in enumerate(pred_boxes):
            if i in tp_preds:
                cv2.rectangle(annotated, (pred[1], pred[2]), (pred[3], pred[4]), TP_COLOUR, 1)
                cv2.putText(annotated, f"TP {pred[5]:.2f}", (pred[1], pred[2] - 6),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, TP_COLOUR, 1)
        for pred in fp_preds:
            cv2.rectangle(annotated, (pred[1], pred[2]), (pred[3], pred[4]), FP_COLOUR, 2)
            name = CLASS_NAMES[pred[0]] if pred[0] < len(CLASS_NAMES) else str(pred[0])
            cv2.putText(annotated, f"FP:{name} {pred[5]:.2f}", (pred[1], pred[2] - 6),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, FP_COLOUR, 2)

        cv2.imwrite(str(out_dir / img_path.name), annotated)

        for pred in fp_preds:
            cls_id, x1, y1, x2, y2, conf = pred
    ...
    if false_positives:
        ...
        for fp in false_positives:
            by_class[fp["class_name"]] = by_class.get(fp["class_name"], 0) + 1
        print("\n  False positives by class:")
        for cls, count in sorted(by_class.items(), key=lambda x: -x[1]):
            print(f"    {cls:<12} {count:>3}  ({round(100*count/total_fp,1)}%)")
