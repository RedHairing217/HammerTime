"""
train.py — Multi-Class Detector
================================
Train YOLOv8-Nano on a multi-class tool dataset prepared by prepare_dataset.py.

Run prepare_dataset.py (no --target_class) first.

Usage
-----
    python src/multi/train_m.py
    python src/multi/train_m.py --epochs 300 --batch 8
    python src/multi/train_m.py --resume
    python src/multi/train_m.py --no_aug
    python src/multi/train_m.py --export
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train YOLOv8-Nano multi-class tool detector",
...
def preflight(args: argparse.Namespace) -> None:
    if not args.resume and not Path(args.data).exists():
        ...
        print("      Run: python src/shared/prepare_dataset.py")
        sys.exit(1)
...
def print_metrics(best_pt: Path) -> None:
    ...
    try:
        ...
        print("\n" + "=" * 48)
        print("  FINAL METRICS  [mean across all classes]")
        print("=" * 48)
        if map50 is not None: print(f"  mAP@50      : {map50:.4f}")
        if map95  is not None: print(f"  mAP@50-95   : {map95:.4f}")
        print("  Run evaluate.py --split test for per-class breakdown.")
        print("=" * 48 + "\n")
...
def main() -> None:
    print("\n╔══════════════════════════════════════════════╗")
    print("║  HammerTime — Multi-Class Detector Training ║")
    print("╚══════════════════════════════════════════════╝\n")
    ...
    print(f"\n[NEXT] Evaluate: python src/multi/evaluate_m.py")
    print(f"       Predict:  python src/multi/predict_m.py --source <image_or_dir>\n")
